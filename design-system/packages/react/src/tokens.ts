/** Genshi public token references (`--gsh-*`). */
...
export function typographyVars(
  prefix: string,
  options?: { includeTextCase?: boolean; includeLineHeight?: boolean }
) {
  ...
  if (options?.includeLineHeight !== false) {
    style.lineHeight = `var(--${prefix}-line-height)`;
  }

  if (options?.includeTextCase) {
    style.textTransform = `var(--${prefix}-text-case)`;
  }
